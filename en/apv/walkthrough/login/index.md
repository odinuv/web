...
<div class="note">
    <p markdown="1">
        ...
        [d'oh](https://en.wikipedia.org/wiki/D'oh!)). Salt is stored directly in the result of password_hash() function.
...
login page with a simple form. After he fills and submits the form, he is verified against the database. If an
existing account is found and passwords match, your application can trust this user.
...
every HTTP request. If the client "forgets" the key, data stored in session is lost. The key is actually called
*session ID*.
...
login pages to prevent attackers to sniff the password from network traffic.
