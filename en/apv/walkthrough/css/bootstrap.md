...
In the previous chapter I explained the [CSS](/en/apv/walkthrough/css/) basics. You should know that CSS is a language which
...
**This text is strongly aimed at Bootstrap software library at version 3.x. It is certain that in future trends will
...
an appropriate widget from their [online library](http://getbootstrap.com/css/). Than you tweak that HTML
...
make sure not to break Bootstrap
...
[jQuery](https://jquery.com) library (also available via [CDN](https://jquery.com/download/#using-jquery-with-a-cdn))
...
{% highlight html %}
{% include /en/apv/walkthrough/css/head-skeleton.html %}
{% endhighlight %}
...
samples in Bootstrap [CSS documentation section](http://getbootstrap.com/css/). Bootstrap has many classes
and applying them on your HTML elements will make those elements look "the Bootstrap way".

For example a button can be styled with `btn` and `btn-primary` [class](http://getbootstrap.com/css/#buttons-options).
...
HTML element. Do not confuse this with a space in a [CSS selector](/en/apv/articles/html/css/#combining-selectors).

Another example is a table which has every second row a bit darker. This can be achieved with following CSS classes:

{% highlight html %}
{% include /en/apv/walkthrough/css/bootstrap-table.html %}
{% endhighlight %}

As you see, Bootstrap CSS classes are split into general and specific.
...
a row which is under the mouse cursor. This approach resembles principle of inheritance/specialization of classes
...
### Task -- Use Bootstrap
Try to apply Bootstrap CSS classes to your application HTML code and make it look modern. Hint: use
`<div class="container">` or `<div class="container-fluid">` element to enclose all your HTML content.

{: .solution}
<div markdown="1">
The solution is different for each of you. Although Bootstrap elements look the same, the placement and
...
Although it is very complicated, Bootstrap is nothing more than CSS. So [all of the CSS rules](/en/apv/articles/html/css/#inheritance--cascading)
still apply. This means that you can easily override anything defined in Bootstrap. However, because Bootstrap is a
[library](/en/apv/articles/programming/#standard-library) (and other people develop it), you should avoid changing anything in the
...
Try to change the default blue color of the primary button to violet (`#935ebc`) and danger button to sharp
red (`#db0808`).

![Screenshot - Overridden button colors](/en/apv/walkthrough/css/button-override.png)
...
{: .solution}
{% highlight html %}
{% include /en/apv/walkthrough/css/bootstrap-override.html %}
{% endhighlight %}
...
usable both on PCs with large screen and also on small handheld devices. Also this must be done preferably with as much shared code
as possible. The key is to apply different CSS properties depending on the screen resolution or other parameters
of the device using a [**media query**](/en/apv/articles/html/css/#at-rules).

You can use a *media query* to apply certain CSS properties only when some conditions are met.

{% highlight css %}
{% include /en/apv/walkthrough/css/media-queries.css %}
{% endhighlight %}
...
{% highlight html %}
{% include /en/apv/walkthrough/css/media-queries.html %}
{% endhighlight %}

You should see that as you resize the browser window the padding will change. I also added a change of background color
...
and also background images (in this above style I set background to plain white and `nav` element not to print).
...
thanks to it you can even create mobile application which can be
