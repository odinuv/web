...
to build web sites for living. PHP has a simple way how to handle [file uploads](http://php.net/manual/en/features.file-upload.php)
...
Now you know that files are being stored in a directory of you choice but you need to store other information to access
...
directory which is accessible via HTTP protocol. Therefore you cannot simply generate a `Location: path-to-file` header
and let browser to do the rest.
...
so the browser can display the file or open appropriate application.
...
{: .note}
`Content-Type` and `Content-Disposition` are also often used when you generate a PDF, PNG/JPG image, Word or Excel
...
You can use JavaScript to check file size or even contents of file on client's computer and to visualize file upload
progress but backend implementation stays practically the same.
