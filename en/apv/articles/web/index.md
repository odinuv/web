...
what *web* really is. Lets start at the very beginning.
...
technology](https://en.wikipedia.org/wiki/OSI_model) used in IT. Computer networks
are standardized on several levels. When you are working on web application
(and not on networking), you don't need to get into too much detail, so it 
...
many situations in IT. For example one may say that:
...
As you see, a subject in the client-server relation does not have to be a computer. Nothing also prevents
a server to someone to become a client to someone else. When working with clients and servers
in context of networks, we therefore talk about applications and roles, not about physical machines.
...
From application point of view, you should interpret the above image as that:
...
together. Each network device is connected through a **network interface**. For example if computer
can have both ethernet cable connection and a WIFI wireless connection, it is connected
...
into logical network is crucial for maintainability, performance and security of the network.
...
unknown services. Notice that on the logical level, there are also some network protocol defined,
but those primarily *service protocols* for running the network and *data transfer protocols*
just for sending the data over.   

### Network protocol
Network protocol is (as any other protocol) a set of rules which are to be followed when 
...
By far the most common protocols are those from **TCP/IP** protocol suite.
...
capital letters, some one invented the term *intranet*. *Intranet* is exactly the same as *internet*,
...
one network interface to another network interface. Other protocols are then built on top of IP.
...
### Internet Addressing
To transmit data between two network devices - *source host* and *destination host*, the 
...
should be low chance of encountering two network interfaces with same MAC address. MAC address is 
unusable in addressing at application level (you would
...
the network administrator (personally or through 
...
physical address of the network interface for the actual delivery of signal.
- Name Address -- assigned manually by machine administrator, usable only for end-users. For communication it
must always be translated to logical address usually through 
...
Socket is combination of *address* and *port* (delimited by colon `:`). Network port is
...
initiates the connection. The servers waits for incoming connection on particular 
network port (because there may multiple servers running on a single computer) -- it **listens on a port**. 
When the client initiates the connection, it must know on what server, the port is listening.
...
is available which assigns port numbers to protocols. So for example, as long as the client
is communicating with HTTP protocol, it expects that the server will be listening on port **80**.
...
Network ports, network protocol and network services are all closely related together. **Network service**
...
not used that much anymore (because the password cannot be encrypted). `address` is either and IP address
...
converts this to `http://www.example.com` because it know he is a web browser and should communicate using
the HTTP protocol. Then the network layer converts this to `http://www.example.com:80`, because `80`
is a well known port for `http` protocol. Then this gets processed by logical network layer and converted
to eg. `http://10.20.30.40:80`. Then the layer will send a request to this destination host and add
source address with ephemeral port to this -- e.g. `http://40.30.20.10:506070`.  
...
- static -- The page content does cannot be change outside from the web server.
- dynamic -- The page content is generated dynamically -- usually from database and 
responds to the end-user actions.

A **web resource** is anything which is provided by the WWW service. It can be a HTML page (only the HTML code),
...
images have the type `image/png`, etc.
...
HTTP is **text** protocol. HTTP request and response consist of **headers** and **body**.
The headers contains metadata about the request and
...
It is important to understand that what the end-user sees a single coherent 
...
a given client, this leads to that there is no *state* retained between the transactions
and all information about the web page state are lost. 
...
responds with successful login page. Then the user request another page (e.g. list of his emails)
...
identify what is a *visit* of page. 

The above can be seen as a tradeoff for the effectiveness of HTTP protocol and HTTP server
-- which is one the reasons it became so popular in the first place. The technology 
dept in this is being addressed by extensions to HTTP protocol or new protocols (such as 
...
are the relations between them. While these are technologies every IT person should be
familiar with, there is often a lot of confusion which I tried to clear up here.
